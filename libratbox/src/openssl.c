...
rb_init_ssl(void)
{
	...
	/* Disable old protocols, make the client use our settings */
	SSL_CTX_set_options(ssl_server_ctx, SSL_OP_NO_SSLv2 | SSL_OP_NO_SSLv3 | SSL_OP_NO_TLSv1 | SSL_OP_CIPHER_SERVER_PREFERENCE);
	SSL_CTX_set_min_proto_version(ssl_server_ctx, TLS1_2_VERSION);
...
rb_setup_ssl_server(const char *cert, const char *keyfile, const char *dhfile)
{
	...
	const char *ciphers = "ECDHE+AESGCM:ECDHE+CHACHA20:DHE+AESGCM:DHE+CHACHA20:HIGH:!aNULL:!MD5:!RC4:!3DES";
	...
	tls_opts |= SSL_OP_NO_SSLv2 | SSL_OP_NO_SSLv3 | SSL_OP_NO_TLSv1 | SSL_OP_NO_TLSv1_1 | SSL_OP_NO_COMPRESSION | SSL_OP_CIPHER_SERVER_PREFERENCE;
	...
	/* OpenSSL 3.x: use automatic ECDH curve selection */
...
rb_get_ssl_info(char *buf, size_t len)
{
	rb_snprintf(buf, len, "Using SSL: %s compiled: 0x%lx, library 0x%lx",
		    OpenSSL_version(OPENSSL_VERSION), OPENSSL_VERSION_NUMBER, OpenSSL_version_num());
}
